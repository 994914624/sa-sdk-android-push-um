    ...
    public static void registerNotificationCallback(PushAgent pushAgent) {
        UmengNotificationClickHandler notificationClickHandler = new UmengNotificationClickHandler() {
            ...
            public void launchApp(Context context, UMessage uMessage) {
                ...
                SFLogger.d(TAG, "launchApp：" + uMessage.custom + "||||" + uMessage.extra);
                // TODO 埋点 App 打开推送消息 事件
                ToolBox.trackAppOpenNotification(uMessage.extra, uMessage.title, uMessage.text);
                // TODO 处理自定义消息
                ToolBox.handleSFCustomMessage(uMessage.extra);


                try {
                    String sf_data = "";
                    if (uMessage.extra != null) {
                        sf_data = new JSONObject(uMessage.extra).optString("sf_data");
                    }
                    ToolBox.handlePush(uMessage.title, uMessage.text, sf_data, context);
                } catch (Exception e) {
                    e.printStackTrace();
                }
